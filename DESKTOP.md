...
**简体中文** · [English](DESKTOP.en.md) · [在线版指南](docs/USAGE.zh-CN.md)

本文件说明 2.1 桌面版；本机服务不可作为 Vercel 后端部署。在线版支持中英文切换，桌面程序界面目前仍为中文。

一个离线优先、本地运行的批量媒体转换工作台。由原 NCM→MP3 工具升级，支持音频、视频、静态图片、GIF 与文本字幕。Windows 主程序使用 Go 编译，启动后打开本机浏览器；**不是云端转换网站，也不是原生 WebView 窗口**。

**交付状态：2.1.0-beta.1 社区预发布，EXE 未签名。** 本次在 Windows x64 上完成 59 项原生媒体回归、12 项浏览器 GUI 检查，以及发布程序启动/实例锁/异常退出恢复检查。38 是“输出预设数”，不代表任意文件与编码组合保证。真实显卡全矩阵、原生选择器人工验收、独立安全及商业合规审查仍未完成。当前记录见 [发布检查](docs/RELEASE.md)；`docs/ACCEPTANCE.md` 为 2.0 历史记录。
...
关闭网页不会结束后台程序，使用右上角“退出”。默认设置和队列保存在 `%APPDATA%\PrismTranscode`。浏览器未打开时，可查看该目录 `last-session-url.txt` 中的本次本机地址；地址仅在对应进程运行期间有效。不要分享其中的会话令牌。2.1 会拒绝第二个进程占用相同数据目录；异常退出后锁由系统释放，不要手动删除 `instance.lock`。
...
最低语言基线是 Go 1.23；当前发布使用 Go 1.27.1。项目没有第三方 Go 模块，桌面版构建不需要 npm；新增在线版的开发构建需要 Node.js 24 LTS。
